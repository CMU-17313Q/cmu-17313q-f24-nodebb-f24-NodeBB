	...
	<div class="post-container d-flex flex-grow-1 flex-column w-100" style="min-width:0;">
		...
		<div class="content mt-2 text-break" component="post/content" itemprop="text">
			{posts.content}
				...
				<span id="reading-time-{posts.pid}" class="text-muted me-3"></span> <!-- Unique ID for each post's reading time -->
		...
		<div class="content mt-2 text-break" component="post/content" itemprop="text" id="post-content-{posts.pid}">
		    {posts.content}
		</div>

				<!-- This was done with assitance of ChatGPT -->
		<script>
		(function() {
			function calculateReadingTime() {
				// Access the post content from the DOM using a unique post ID
				const postContentElement = document.getElementById('post-content-{posts.pid}');
				const readingTimeElement = document.getElementById('reading-time-{posts.pid}');  // Unique post ID

				if (postContentElement && readingTimeElement) { // Check if the element exists
					const postContent = postContentElement.innerText || postContentElement.textContent;  // Extract text content

					// Calculate the word count
					const wordCount = postContent.trim().split(/\s+/).length;

					// Estimate the reading time (rounded up)
					const readingTime = Math.ceil(wordCount / 200);

					// Display the estimated reading time correctly
					readingTimeElement.textContent = 'Estimated reading time: ' + readingTime + ' min';  // Proper display
				}
			}
			...
			// Also bind to the ajaxify event in case of page navigation
			$(window).on('action:ajaxify.content', function() {
				calculateReadingTime();
			});
		})();
		</script>
...
<div component="post/footer" class="post-footer border-bottom pb-2">
	...
	<div component="post/actions" class="d-flex justify-content-end gap-1 post-tools">
		...
		{{{ end }}}
		<button component="post/toggle-button" class="btn-ghost-sm" data-toggle="post-toggle">
